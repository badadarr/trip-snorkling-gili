...
export default function AdminPackagesPage() {
  ...
  const updateScheduleFromTimes = (t1: string, t2: string, has2: boolean, flex: boolean) => {
    if (flex) {
      setFormData((prev) => ({
        ...prev,
  ...
  const updateDurationFromHours = (minH: number, maxH: number, flex: boolean) => {
  ...
  const openEditModal = (pkg: any) => {
    ...
    setFormData({
      ...
      priceUnit: pkg.priceUnit || (pkg.price > 500000 ? 'per_boat' : 'per_person'),
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '14px' }}>
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <input
                    type="text"
                    className="form-control"
                    placeholder="contoh: private-glass-bottom-boat"
                    value={formData.slug}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div className="form-group" style={{ margin: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    <label className="form-label" style={{ margin: 0 }}>Harga IDR (Rupiah) *</label>
                    {formData.price > 0 && (
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--primary-ocean)' }}>
                        {formatIdr(formData.price)}
                      </span>
                    )}
                  </div>
                  <div style={{ position: 'relative' }}>
                    <span style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', fontWeight: 700, fontSize: '0.85rem' }}>
                      Rp
                    </span>
                    <input
                      type="text"
                      ...
                      onChange={(e) => {
                        ...
                        if ((formData.priceUsd === 0 || !formData.priceUsd) && num > 0) {
                ...
                <div className="form-group" style={{ margin: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    <label className="form-label" style={{ margin: 0 }}>Harga USD ($) *</label>
                    {formData.priceUsd > 0 && (
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (separatorIndex !== -1) {
                          ...
                          const after = raw.slice(separatorIndex + 1).replace(/[.,]/g, '');
                          raw = before + after;
                        }
                        setUsdInputStr(raw);
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <label
                    style={{
                      ...
                      border: formData.priceUnit === 'per_person' ? '2px solid var(--primary-ocean)' : '1px solid var(--border-light)',
                      background: formData.priceUnit === 'per_person' ? 'var(--primary-surface)' : '#f8fafc',
                      cursor: 'pointer',
                    }}
                  >
                    <input
                      ...
                      onChange={() => setFormData({ ...formData, priceUnit: 'per_person' })}
                      style={{ marginTop: '3px' }}
                    />
                    <div>
                      <strong style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.88rem', color: 'var(--primary-deep)' }}>
                        <User size={15} color="var(--primary-ocean)" />
                        <span>Per Orang (Per Person)</span>
                      </strong>
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', lineHeight: 1.3, display: 'block', marginTop: '2px' }}>
                  ...
                  <label
                    style={{
                      ...
                      padding: '12px 14px',
                      borderRadius: 'var(--radius-sm)',
                      border: formData.priceUnit === 'per_boat' ? '2px solid var(--primary-ocean)' : '1px solid var(--border-light)',
                      background: formData.priceUnit === 'per_boat' ? 'var(--primary-surface)' : '#f8fafc',
                      cursor: 'pointer',
                    }}
                  >
                    <input
                      ...
                      onChange={() => setFormData({ ...formData, priceUnit: 'per_boat' })}
                      style={{ marginTop: '3px' }}
                    />
                    <div>
                      <strong style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.88rem', color: 'var(--primary-deep)' }}>
              ...
              >
                ...
                <div style={{ marginBottom: '18px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <label className="form-label" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 700 }}>
                      <Clock size={15} color="var(--primary-ocean)" />
                      <span>Durasi Trip Snorkeling *</span>
                    </label>
                    {formData.durationId && (
                      <span style={{ fontSize: '0.78rem', padding: '2px 8px', borderRadius: '4px', background: '#e0f2fe', color: 'var(--primary-ocean)', fontWeight: 700 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                    {[
                      ...
                      { label: '4 - 5 Jam (Standar)', min: 4, max: 5, flex: false },
                      { label: '4 - 5 Jam (Fleksibel)', min: 4, max: 5, flex: true },
                      { label: '6 - 7 Jam (Full Day)', min: 6, max: 7, flex: false },
                    ].map((preset, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => {
                          ...
                          updateDurationFromHours(preset.min, preset.max, preset.flex);
                        }}
                        style={{
                          padding: '5px 10px',
                          borderRadius: '6px',
                          border: formData.durationId.startsWith(`${preset.min}`) ? '1.5px solid var(--primary-ocean)' : '1px solid var(--border-light)',
                          background: formData.durationId.startsWith(`${preset.min}`) ? 'var(--primary-surface)' : '#ffffff',
                          color: formData.durationId.startsWith(`${preset.min}`) ? 'var(--primary-ocean)' : 'var(--text-main)',
                          fontSize: '0.78rem',
                          fontWeight: 600,
                          cursor: 'pointer',
                        }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#ffffff', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--border-light)' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Atur Manual:</span>
                    <input
                      type="number"
                      step="0.5"
                      min="1"
                      ...
                      onChange={(e) => {
                        ...
                        updateDurationFromHours(val, durationHoursMax, durationIsFlexible);
                      }}
                    />
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateDurationFromHours(durationHoursMin, val, durationIsFlexible);
                      }}
                    />
                    <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>Jam</span>

                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', marginLeft: 'auto', fontSize: '0.78rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={durationIsFlexible}
                        onChange={(e) => {
                          setDurationIsFlexible(e.target.checked);
                          updateDurationFromHours(durationHoursMin, durationHoursMax, e.target.checked);
                        }}
                      />
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                    ...
                    {formData.scheduleId && (
                      <span style={{ fontSize: '0.78rem', padding: '2px 8px', borderRadius: '4px', background: '#fef3c7', color: '#b45309', fontWeight: 700 }}>
                        {formData.scheduleId}
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                    {[
                      { label: 'Pagi: 09:00 WITA', t1: '09:00', t2: '', has2: false, flex: false },
                      { label: 'Pagi: 09:30 WITA', t1: '09:30', t2: '', has2: false, flex: false },
                      { label: 'Siang: 13:30 WITA', t1: '13:30', t2: '', has2: false, flex: false },
                      { label: 'Sunset: 15:30 WITA', t1: '15:30', t2: '', has2: false, flex: false },
                      { label: '2 Sesi: 09:30 & 13:30 WITA', t1: '09:30', t2: '13:30', has2: true, flex: false },
                      { label: 'Bebas / Fleksibel (Private)', t1: '', t2: '', has2: false, flex: true },
                    ].map((preset, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => {
                          ...
                          updateScheduleFromTimes(preset.t1, preset.t2, preset.has2, preset.flex);
                        }}
                        style={{
                          padding: '5px 10px',
                          borderRadius: '6px',
                          border: formData.scheduleId.includes(preset.t1 || 'Fleksibel') ? '1.5px solid #d97706' : '1px solid var(--border-light)',
                          background: formData.scheduleId.includes(preset.t1 || 'Fleksibel') ? '#fffbeb' : '#ffffff',
                          color: formData.scheduleId.includes(preset.t1 || 'Fleksibel') ? '#b45309' : 'var(--text-main)',
                          fontSize: '0.78rem',
                          fontWeight: 600,
                          cursor: 'pointer',
                        }}
                  ...
                  {/* Time Inputs Picker */}
                  {!isScheduleFlexible ? (
                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px', background: '#ffffff', padding: '10px 12px', borderRadius: '6px', border: '1px solid var(--border-light)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>Jam Sesi 1:</span>
                        <input
                          type="time"
                          className="form-control"
                          style={{ width: '125px', padding: '4px 8px', fontSize: '0.85rem' }}
                          value={timePicker1}
                          onChange={(e) => {
                            const val = e.target.value;
                            setTimePicker1(val);
                            updateScheduleFromTimes(val, timePicker2, hasSecondSlot, false);
                          }}
                        />
                      </div>
                      ...
                      {hasSecondSlot && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>Jam Sesi 2:</span>
                          <input
                            type="time"
                            className="form-control"
                            style={{ width: '125px', padding: '4px 8px', fontSize: '0.85rem' }}
                            ...
                            onChange={(e) => {
                              const val = e.target.value;
                              setTimePicker2(val);
                              updateScheduleFromTimes(timePicker1, val, true, false);
                            }}
                          />
                        </div>
                      )}
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          updateScheduleFromTimes(timePicker1, timePicker2, next, false);
                        }}
                        style={{
                          marginLeft: 'auto',
                          padding: '4px 10px',
                          borderRadius: '6px',
                          border: '1px solid var(--border-light)',
                          background: hasSecondSlot ? '#fee2e2' : 'var(--primary-surface)',
                          color: hasSecondSlot ? '#b91c1c' : 'var(--primary-ocean)',
                          fontSize: '0.76rem',
                          fontWeight: 700,
                          cursor: 'pointer',
                        }}
                    ...
                    <div style={{ padding: '8px 12px', background: '#fffbeb', borderRadius: '6px', fontSize: '0.8rem', color: '#b45309', fontWeight: 600 }}>
                      Waktu Keberangkatan Bebas & Fleksibel (Private Boat On-Demand)
                    </div>
                  )}
                </div>
              </div>
