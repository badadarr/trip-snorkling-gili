...
interface BookingDetailModalProps {
  ...
  onUpdateNotes?: (id: number, specialRequests: string, pickupLocation: string) => Promise<void>;
}
...
export default function BookingDetailModal({
  ...
  const [activeTemplate, setActiveTemplate] = useState<'confirm' | 'reminder' | 'meeting' | 'cancel'>('confirm');
  const [customMessage, setCustomMessage] = useState('');
  const [copied, setCopied] = useState(false);
  const [isUpdatingStatus, setIsUpdatingStatus] = useState(false);
  const [isEditingNotes, setIsEditingNotes] = useState(false);
  const [pickupLoc, setPickupLoc] = useState(booking.pickupLocation || '');
  ...
  // Templates Generator
  const generateTemplateText = (tpl: 'confirm' | 'reminder' | 'meeting' | 'cancel') => {
    const sessionName =
  ...
  const handleSelectTemplate = (tpl: 'confirm' | 'reminder' | 'meeting' | 'cancel') => {
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h2 style={{ fontSize: '1.25rem', color: 'var(--primary-deep)', margin: 0 }}>
                Rincian Reservasi: <span style={{ color: 'var(--primary-ocean)' }}>{booking.bookingCode}</span>
              </h2>
            ...
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              Dipesan pada {booking.createdAt ? new Date(booking.createdAt).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}
            </span>
          </div>
        ...
        {/* Modal Body */}
        <div style={{ padding: '24px 28px', display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            >
              <label style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--primary-deep)', display: 'block', marginBottom: '8px' }}>
                STATUS RESERVASI
              </label>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {[
                  { key: 'pending', label: 'Pending', bg: '#fef3c7', text: '#b45309' },
                  { key: 'confirmed', label: 'Confirmed', bg: '#d1fae5', text: '#065f46' },
                  { key: 'completed', label: 'Completed', bg: '#e0f2fe', text: '#0369a1' },
                  { key: 'cancelled', label: 'Cancelled', bg: '#fee2e2', text: '#b91c1c' },
                ].map((st) => (
                  <button
                    key={st.key}
                    type="button"
                    ...
                    style={{
                      ...
                      border: booking.status === st.key ? `2px solid ${st.text}` : '1px solid var(--border-light)',
                      backgroundColor: booking.status === st.key ? st.bg : '#ffffff',
                      color: st.text,
                      fontSize: '0.82rem',
                      fontWeight: 700,
            ...
            <div className="glass-card" style={{ padding: '18px', background: '#ffffff' }}>
              <h4 style={{ fontSize: '0.92rem', color: 'var(--primary-deep)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '0.88rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <strong style={{ color: 'var(--primary-deep)' }}>{booking.customerName}</strong>
                </div>
            ...
            <div className="glass-card" style={{ padding: '18px', background: '#ffffff' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '0.88rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Paket Snorkeling:</span>
                  <strong style={{ color: 'var(--primary-deep)', textAlign: 'right', maxWidth: '60%' }}>
                    {booking.packageName}
                  </strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Tanggal Keberangkatan:</span>
                  <strong style={{ color: 'var(--primary-ocean)' }}>{booking.tripDate}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Jumlah Peserta:</span>
                ...
                >
                  <span style={{ fontWeight: 700, color: 'var(--primary-deep)' }}>Total Tagihan:</span>
                  <strong style={{ fontSize: '1.1rem', color: 'var(--primary-ocean)' }}>
            ...
            <div className="glass-card" style={{ padding: '18px', background: '#ffffff' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                <h4 style={{ fontSize: '0.92rem', color: 'var(--primary-deep)', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {!isEditingNotes ? (
                <div style={{ fontSize: '0.85rem', color: 'var(--text-main)', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <div>
                    <span style={{ color: 'var(--text-muted)' }}>Titik Kumpul / Pickup: </span>
                    <span>{booking.pickupLocation || 'Dermaga Utama Gili Trawangan'}</span>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-muted)' }}>Permintaan Khusus: </span>
                    <span style={{ fontStyle: booking.specialRequests ? 'normal' : 'italic' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
                  ...
                  <textarea
                    ...
                    rows={2}
                    style={{ padding: '8px 12px', fontSize: '0.85rem', minHeight: '60px' }}
                  />
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                    ...
                    >
                      {isSavingNotes && <Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} />}
                      <span>Simpan</span>
                    </button>
                  </div>
                </div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
              <div style={{ padding: '6px', borderRadius: '8px', background: 'rgba(37, 211, 102, 0.15)', color: '#15803d' }}>
                <MessageCircle size={18} />
              </div>
              <div>
                <h4 style={{ fontSize: '0.95rem', color: 'var(--primary-deep)', margin: 0 }}>
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    padding: '8px 10px',
                    borderRadius: 'var(--radius-sm)',
                    border: activeTemplate === t.key ? '1.5px solid #15803d' : '1px solid var(--border-light)',
                    backgroundColor: activeTemplate === t.key ? 'rgba(37, 211, 102, 0.15)' : '#ffffff',
                    color: activeTemplate === t.key ? '#15803d' : 'var(--text-main)',
                    fontSize: '0.8rem',
                    fontWeight: 700,
                    cursor: 'pointer',
                    textAlign: 'center',
