...
export default function HeroSection({ heroData }: HeroProps) {
  ...
  const badge = heroData?.badgeEn || t('popularTag');
  const title = heroData?.titleEn || t('mainHeading');
  const subtitle = heroData?.subtitleEn || t('subHeading');
  const ctaText = heroData?.ctaTextEn || t('ctaBook');
  const secondaryCtaText = heroData?.secondaryCtaEn || t('ctaPackages');
